...
class ObjectSearchQNetwork(nn.Module):
    ...
    def __init__(self, in_channels: int = 6, num_actions: int = 3, feature_dim: int = 256):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(in_channels, 32, kernel_size=5, stride=2, padding=2), nn.ReLU(),   # (6,35,35) -> (32,18,18)
            nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1),          nn.ReLU(),   # -> (64,9,9)
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),          nn.ReLU(),   # -> (64,9,9)
        )
        self.fc = nn.Sequential(
            nn.Linear(64*9*9, feature_dim),  # 64*9*9 = 5184
            nn.ReLU(),
